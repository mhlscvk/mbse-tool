...
/**
 * Sync a single examples directory tree as a system project owned by the
 * system user.  Used for both English (Examples) and Turkish (Örnekler).
 */
async function syncExamplesFrom(opts: {
  sourceDir: string;
  rootName: string;
  systemUserId: string;
}): Promise<{ dirs: number; files: number }> {
  const { sourceDir, rootName, systemUserId } = opts;
  if (!existsSync(sourceDir)) return { dirs: 0, files: 0 };
  ...
  if (!root) {
    root = await prisma.project.create({
      data: { name: rootName, displayId: generateProjectDisplayId('SYSTEM', '0001'), ownerId: systemUserId, parentId: null, depth: 0, isSystem: true, projectType: 'SYSTEM' },
    });
  }

  const dirs = readdirSync(sourceDir).filter(e => statSync(join(sourceDir, e)).isDirectory());
  let totalFiles = 0;

  for (const dirName of dirs) {
    const dirPath = resolve(sourceDir, dirName);
    if (!dirPath.startsWith(sourceDir)) continue;
    ...
    if (sub) {
      ...
      sub = await prisma.project.create({
        data: { name: dirName, displayId: generateProjectDisplayId('SYSTEM', '0001'), ownerId: systemUserId, parentId: root.id, depth: 1, isSystem: true, projectType: 'SYSTEM' },
      });
    }
    ...
    const fileData = files.map(fileName => {
      ...
      if (!filePath.startsWith(sourceDir)) return null;
  ...
  return { dirs: dirs.length, files: totalFiles };
}

// POST /api/admin/sync-examples — re-import examples from prisma/examples/ AND prisma/examples-tr/
router.post('/sync-examples', asyncHandler(async (_req: AuthRequest, res) => {
  ...
  const en = await syncExamplesFrom({ sourceDir: EXAMPLES_DIR, rootName: 'Examples', systemUserId: systemUser.id });
  const tr = await syncExamplesFrom({ sourceDir: EXAMPLES_TR_DIR, rootName: 'Örnekler', systemUserId: systemUser.id });

  res.json({ data: { message: `Examples synced: ${en.dirs + tr.dirs} subprojects (EN: ${en.dirs}, TR: ${tr.dirs}), ${en.files + tr.files} files` } });
}));
